...
export async function initializeClients(userEmail: string) {
  try {
    ...
    if (spaces.length > 0) {
      ...
      // Try to get storage usage through the agent
      const storageUsage = await space.agent?.getStorageUsage?.() || {
        used: 0,
        total: 0,
        available: 0
      };
      ...
      console.log("Storage usage:", {
        ...
        total: storageUsage.total || 0,
        available: storageUsage.available || 0
      });
